    ...
    <DashboardLayout>
      <div className="page-container py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard label="Regions" value={16} icon={ChartBarIcon} color="bg-blue-500" to="/admin/regions" />
        </div>
        ...
        {needsAttention && (
          <div className="card p-6 mb-8 border-amber-200 dark:border-amber-800/40 bg-amber-50/50 dark:bg-amber-900/10">
            ...
            <div className="space-y-3">
              {hasPendingVendors && (
                <div className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-xl border border-amber-200 dark:border-amber-800/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Review documents and approve or reject vendor accounts</p>
                    </div>
                  </div>
                  <Link
                    to="/admin/vendors"
              ...
              {hasFlaggedProducts && (
                <div className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-xl border border-red-200 dark:border-red-800/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Products reported by users or flagged by the system</p>
                    </div>
                  </div>
                  <Link
                    to="/admin/products"
            ...
            <div className="space-y-3">
              {data?.recentVendors?.length > 0 ? data.recentVendors.map((v) => (
                <div key={v._id} className="flex items-center gap-3 py-2 border-b border-gray-50 dark:border-gray-800 last:border-0">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${v.isApproved ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                    {v.isApproved ? 'Approved' : 'Pending'}
                  </span>
                </div>
              )) : (
                <p className="text-sm text-gray-400 py-6 text-center">No vendors yet</p>
              )}
            </div>
          </div>
        ...
        {/* Platform health */}
        <div className="card p-6">
          <h2 className="font-bold text-gray-900 dark:text-gray-100 mb-4">Platform Health</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              },
              {
                ...
                color: hasPendingVendors ? 'text-amber-600 dark:text-amber-400' : 'text-gray-600 dark:text-gray-400',
                bg: hasPendingVendors ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-50 dark:bg-gray-800',
              },
              {
                label: 'Flagged Content',
                value: stats.flaggedProducts || 0,
                color: hasFlaggedProducts ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400',
                bg: hasFlaggedProducts ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-800',
              },
              {
